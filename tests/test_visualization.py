...
# ---------------------------------------------------------------------------
# Schema-fallback mesh generation tests (no PyVista required)
# ---------------------------------------------------------------------------

def _make_fake_pyvista() -> ModuleType:
    """Build a lightweight PyVista stub so mesh helpers can run without PyVista."""
    ...
    class FakeBox:
        ...
        def rotate_vector(self, axis, angle, inplace=False):
            return self
    ...
    pv.Box.side_effect = lambda bounds: FakeBox(bounds)
    pv.Sphere.side_effect = lambda radius, center: FakeSphere(radius=radius, center=center)
    return pv


class TestSchemaMeshForShape:
    @pytest.fixture(autouse=True)
    def _patch_pyvista(self):
        fake_pv = _make_fake_pyvista()
        with patch.dict(sys.modules, {"pyvista": fake_pv}):
            yield

    def test_bounding_box_returns_mesh(self, fluid_config):
        from sphinxsim.visualization.preview import _schema_mesh_for_shape

        shape = fluid_config.geometries.shapes[0]  # WaterBody bounding_box
        mesh = _schema_mesh_for_shape(shape, fluid_config)
        assert mesh is not None

    def test_box_with_transform_returns_mesh(self):
        from sphinxsim.visualization.preview import _schema_mesh_for_shape
        from sphinxsim.config.schemas import ShapeConfig

        shape = ShapeConfig(
            name="RotatedBox",
            type="box",
            half_size=[0.5, 0.25],
            transform={"translation": [1.0, 2.0], "rotation_angle": 0.785},
        )
        mesh = _schema_mesh_for_shape(shape, None)
        assert mesh is not None

    def test_expanded_box_returns_mesh(self, fluid_config):
        from sphinxsim.visualization.preview import _schema_mesh_for_shape
        from sphinxsim.config.schemas import ShapeConfig

        # WallBoundary (bounding_box) is a valid "original"
        shape = ShapeConfig(
            name="BigBox",
            type="expanded_box",
            original="WallBoundary",
            expansion=0.1,
        )
        mesh = _schema_mesh_for_shape(shape, fluid_config)
        assert mesh is not None

    def test_triangle_mesh_returns_none(self):
        from sphinxsim.visualization.preview import _schema_mesh_for_shape
        from sphinxsim.config.schemas import ShapeConfig

        shape = ShapeConfig(name="STL", type="triangle_mesh", file_path="part.stl")
        mesh = _schema_mesh_for_shape(shape, None)
        assert mesh is None

    def test_complex_shape_returns_none(self, fluid_config):
        from sphinxsim.visualization.preview import _schema_mesh_for_shape
        from sphinxsim.config.schemas import ShapeConfig
        ...
        shape = ShapeConfig(
            name="Compound",
            type="complex_shape",
            sub_shapes=["WaterBody"],
            operations=["union"],
        )
        mesh = _schema_mesh_for_shape(shape, fluid_config)
        assert mesh is None

    def test_multipolygon_with_polygon_bounds(self):
        from sphinxsim.visualization.preview import _schema_mesh_for_shape
        from sphinxsim.config.schemas import ShapeConfig
...
class TestSchemaMeshForOrientedBox:
    @pytest.fixture(autouse=True)
    def _patch_pyvista(self):
        fake_pv = _make_fake_pyvista()
        with patch.dict(sys.modules, {"pyvista": fake_pv}):
    ...
    def test_in_outlet_2d_returns_mesh(self, fluid_config):
        from sphinxsim.visualization.preview import _schema_mesh_for_oriented_box

        ob = fluid_config.geometries.oriented_boxes[0]  # Inlet in_outlet
        mesh = _schema_mesh_for_oriented_box(ob)
        assert mesh is not None

    def test_region_2d_returns_mesh(self):
        from sphinxsim.visualization.preview import _schema_mesh_for_oriented_box
        from sphinxsim.config.schemas import OrientedBoxConfig

        ob = OrientedBoxConfig(
            name="Zone",
            type="region",
            half_size=[0.2, 0.1],
            transform={"translation": [0.5, 0.5], "rotation_angle": 0.0},
        )
        mesh = _schema_mesh_for_oriented_box(ob)
        assert mesh is not None

    def test_in_outlet_missing_center_returns_none(self):
        from sphinxsim.visualization.preview import _schema_mesh_for_oriented_box
        from sphinxsim.config.schemas import OrientedBoxConfig

        # Missing center — validator would normally reject, but we test the
        # mesh helper defensively
        ob = OrientedBoxConfig.__new__(OrientedBoxConfig)
        object.__setattr__(ob, "name", "X")
        object.__setattr__(ob, "type", type("T", (), {"value": "in_outlet"})())
        object.__setattr__(ob, "center", None)
        object.__setattr__(ob, "normal", None)
        object.__setattr__(ob, "radius", None)
        object.__setattr__(ob, "half_size", None)
        object.__setattr__(ob, "transform", None)
